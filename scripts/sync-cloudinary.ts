...
interface CloudinaryAsset extends CloudinaryResource {
  created_at: string;
  folder: string;
  context?: {
...
async function syncSiteImages() {
  try {
    ...
    // Create a map of base public_id -> mobile resource
    const mobileMap = new Map<string, CloudinaryResource>(
      mobileResources.map((r: CloudinaryResource) => [
    ...
    for (const [index, resource] of desktopResources.entries()) {
      ...
      await db.insert(images).values({
        ...
        imageType,
        variant: hasMobileVariant ? 'desktop' : 'both',
        altText,
        caption: null,
        position: index,
      ...
      if (hasMobileVariant) {
        ...
        if (mobileResource) {
          await db.insert(images).values({
            ...
            imageUrl: mobileResource.secure_url,
            imageType,
            variant: 'mobile',
            altText,
            caption: null,
            position: index,
            width: mobileResource.width,
            height: mobileResource.height,
            format: mobileResource.format,
          });
    ...
    console.log(`\n✅ Synced ${insertCount} site image records\n`);
  } catch (error: unknown) {
    if (
      typeof error === 'object' &&
      error !== null &&
  ...
  try {
    for (const [index, resource] of imageResources.entries()) {
      ...
      if (hasMobileVariant) {
        await db.insert(images).values({
          projectSlug,
          publicId: mobileVariant.public_id,
          imageUrl: mobileVariant.secure_url,
          imageType,
          variant: 'mobile',
          altText,
          caption: null,
