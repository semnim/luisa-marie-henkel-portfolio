...
function CreditItem({ label, value }: CreditItemProps) {
  return (
    <div className="flex flex-col gap-1 my-auto">
      <dt className="text-xl md:text-2xl text-left tracking-wide text-foreground font-light">
        {label}
      </dt>
      {typeof value === 'string' && (
        <dd className="text-left text-md md:text-lg text-muted-foreground">
          {value}
        </dd>
      )}
      {Array.isArray(value) && (
        <dd className="text-md text-left text-md md:text-lg text-muted-foreground">
          <ul>
            {value.map((val) => (
              <li key={val}>{val}</li>
            ))}
...
export function ProjectMetadata({
  ...
  return (
    <section className="p-8 lg:p-32 w-full snap-center md:snap-proximity h-dvh flex flex-col">
      <h2 className="text-4xl text-center font-light tracking-wide">
        ABOUT THIS PROJECT
      </h2>
      <div className="mx-auto mt-8 px-4 max-w-2xl flex flex-col flex-1">
        {description && (
          <p className="leading-relaxed text-justify text-sm text-muted-foreground">
            {description}
          </p>
        )}
        {/* Credits Grid */}
        <div className="my-auto flex-1 flex items-start mt-32">
          <dl className="flex-1 grid grid-cols-1  gap-6  mx-auto">
